...
!	   implicit none
       ...
       REAL:: DLY(nmax),GAMA1J(mmax)
       INTEGER:: IP(max)

       open(6,FILE='GRID-N.OUT')
       OPEN(7,FILE='CTLP-N.OUT')
       OPEN(8,FILE='INFLU-N.OUT')
       OPEN(9,FILE='VEL-N.OUT')
       OPEN(10,FILE='MATRX-N.OUT')
...
!      WING VORTEX LATTICE LISTING
       OPEN(11, FILE='GAMA-N.DAT')
       WRITE(11,60)
60     FORMAT(3X,'J'6X,'GAMA(I,J)')
...
!      FORCES CALCULATION
	   ...
	   DO 20 J=1,JB
	      ...
	      DO I=1,IB
	         ...
	         IF(I.GT.1) GAMA(I,J)=GAMA(I,J)-GAMA(I-1,J)
...
!         OUTPUT
	      ...
	      DO 21 J=1,JB
             ...
             END DO 
             DLYJ=DLY(J)/B*JB
             WRITE(6,103) J,DLYJ,DP(1,J),DP(2,J),DP(3,J),DP(4,J),     &
                          GAMA(1,J),GAMA1J(1),GAMA1J(2),GAMA1J(3),GAMA1J(4)
21        CONTINUE  
...
100       CONTINUE
          ...
          WRITE(6,500)
500       FORMAT(//,2X,'J',5X,'QC(I,J,1)',3X,'QC(I,J,2)',3X,'QC(I,J,3)',3X,'DP(I,J)')
600       FORMAT(I3,4F12.4)

	      DO J=1,JB
	         DO I=2,IB
             WRITE(6,600) J, QC(I,J,1), QC(I,J,2), QC(I,J,3), DP(I,J)
             END DO
             WRITE(6,*) 
          END DO  
...
103       FORMAT(1X,I3,3X,F9.3,3X,4F9.3,3X,5F9.3)

104       FORMAT(/,10X,'CL=',F10.4,2X,'FL=',F10.4,4X,'CM=',F10.4,3X, &
                 'CD=',F10.4)

110       FORMAT(//,105('='),/,3X,'J',7X,'DLY',9X,'DCP-1',4X,'DCP-2',4X,'DCP-3',&
                4X,'DCP-4',7X,'GAMA',4X,'GAMA1-1',2X,'GAMA1-2',2X,'GAMA1-3',  &
			    2X,'GAMA1-4',/,105('='))
...
!	        QF(I,J,1)=QF(I,1,1)*CHORD+DXLE
	        ...
	        QF(I,J,1)=(QF(I,1,1)*CHORD+DXLE*(I-0.75))        !CS1
...
!------>      COMPUTATION OF CHORDWISE VECTORS DS(IJ,1,2,3), TANGENTIAL AND
!------>      NORMAL VECTORS DS(IJ,4 TO 9),PANEL AREA DS(IJ,1-10)
