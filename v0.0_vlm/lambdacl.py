...
f = open('outputdata/lambdacl.txt', 'r')
...
plt.title("Lift Coefficient vs. Sweep Angle")
plt.text(-max(x_list)*0.95,max(y_list)*0.95,data[0],fontsize=10)
plt.text(-max(x_list)*0.95,max(y_list)*0.93,data[1],fontsize=10)
plt.xlabel("Sweep Angle")
plt.ylabel("Lift Coefficient, CL")
plt.grid()

savefig('figures/lambdacl.png')
